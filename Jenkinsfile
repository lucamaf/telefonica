...
node() {
  stage('Pre-requisites') {
    if (params.OPENAPI_SPECIFICATION_FILE != "echo-api.json"  ) {
  ...
  stage("Deploy API in Dev") {
    ...
    service = toolbox.prepareThreescaleService(
        ...
        service: [:],
        applications: [
            [ name: "my-app", description: "This is used for tests", plan: "test", account: params.DEV_DEVELOPER_ACCOUNT_ID ]
        ],
        applicationPlans: [
...
def runIntegrationTests(def service) {
  ...
  ${getCredentialsCodeSnippet}
  curl -sfk -w "Get Echo answer: %{http_code}\n" -o /dev/null "${proxy.sandbox_endpoint}/test" -H "\$credential_header"
  """
